...
ruta=os.path.join(os.path.dirname(__file__), "..\\modules\\complements\\complements\\bin\\Debug\\netcoreapp3.1\\complements.exe")
...
def tarea(args):
    ...
    socketIO.emit('endScrape', (const.COMPLEMENTS_SOCKET_TYPE, args[0]))  #emite evento para decirle al server que ya termino la tarea
...
socketIO.emit('connect-socket', const.COMPLEMENTS_SOCKET_TYPE)
